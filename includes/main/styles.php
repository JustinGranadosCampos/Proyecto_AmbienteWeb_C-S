<?php
    echo '<link rel="shortcut icon" href="#">';
    echo '<link rel="stylesheet" href="../../css/bootstrap/css/bootstrap.min.css">';
    echo '<link rel="stylesheet" href="../../css/global.css">';
    echo '<link href="https://fonts.googleapis.com/css2?family=Exo:ital,wght@1,200;1,400&display=swap" rel="stylesheet">';
    echo '<link rel="stylesheet" href="../../vendor/DataTables/datatables.min.css">';
    echo '<link rel="stylesheet" href="../../vendor/DataTables/DataTables-1.10.22/css/dataTables.bootstrap4.min.css">';
    echo '<link rel="stylesheet" href="https://cdn.datatables.net/buttons/1.7.0/css/buttons.dataTables.min.css">';
    echo '<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-select/1.13.18/css/bootstrap-select.min.css ">';
    echo '<script src="https://unpkg.com/sweetalert/dist/sweetalert.min.js"></script>';
?>